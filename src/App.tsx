...
function ContractEditorRoute({ children }: { children: React.ReactNode }) {
  ...
  return ['sales', 'admin', 'creative', 'linen', 'stockroom'].includes(user.role)
    ? <>{children}</>
    : <Navigate to="/contracts" replace />;
}
...
function AppRoutes() {
  const { user } = useAuth();
  const contractAccessRoles = ['sales', 'accounting', 'creative', 'linen', 'stockroom', 'kitchen', 'banquet_supervisor', 'logistics', 'admin'];
  ...
  return (
    <Routes>
      ...
      <Route path="/contracts" element={
        <RoleRoute allowedRoles={contractAccessRoles}>
          <Contracts />
        </RoleRoute>
      } />
      ...
      <Route path="/signed-contracts" element={
        <RoleRoute allowedRoles={['sales', 'admin']}>
          <SignedContracts />
        </RoleRoute>
      } />
