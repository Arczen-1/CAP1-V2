...
interface InventorySummaryItem {
  ...
  notes?: string;
  blockers: string[];
  alternativeSuggestions?: InventoryAlternativeSuggestion[];
}

interface InventoryAlternativeSuggestion {
  itemId: string;
  itemName: string;
  itemCode?: string;
  category?: string;
...
const resolveFinalPaymentPercent = (downPaymentValue?: number, finalPaymentValue?: number) => {
  if (Number(downPaymentValue) === 60 && Number(finalPaymentValue) === 40) {
    return 60;
  }
...
const getFortyPercentDueDate = (bookingDate?: string) => {
  const dueDate = new Date(bookingDate || new Date());
  dueDate.setMonth(dueDate.getMonth() + 2);
  return dueDate;
};
...
export default function ContractDetail() {
  ...
  const handleApprove = async () => {
    ...
    if (finalBalancePastDue) {
      ...
      toast.info(`The final 60% balance must be settled by ${finalPaymentDueDate.toLocaleDateString()} or the event cannot proceed.`);
      return;
    }
    ...
    if (paymentBalance > 0) {
      ...
      toast.info(
        fullPaymentRequired
          ...
          : `Confirm the required ${paymentRequirementPercent}% collection milestone of ${formatCurrency(paymentRequired)} before approving this contract for preparation.`
      );
      return;
    }
  ...
  const handleUpdatePaymentPlan = async (plan: 'split' | 'full') => {
    ...
    const nextDownPaymentPercent = plan === 'full' ? 100 : 40;
    const nextFinalPaymentPercent = plan === 'full' ? 0 : 60;
    const planChanged = (
    ...
    try {
      ...
      toast.success(plan === 'full' ? 'Full payment term confirmed.' : '40 / 60 payment term confirmed.');
      fetchContractData();
    } catch (error: any) {
  ...
  const openProcurementRequestDialog = (section: InventorySectionKey, item: InventorySummaryItem) => {
    ...
    const eventDaysAway = Math.ceil((new Date(contract.eventDate).setHours(0, 0, 0, 0) - new Date().setHours(0, 0, 0, 0)) / (1000 * 60 * 60 * 24));
    setProcurementRequestForm({
      requestType: item.sameDayConflict ? 'rental' : 'purchase',
      requisitionType: eventDaysAway < 7 || item.sameDayConflict ? 'emergency_requisition' : 'purchase_requisition',
  ...
  const handleSubmitProcurementRequest = async () => {
    ...
    try {
      await api.createProcurementRequest({
        ...
        requisitionType: procurementRequestForm.requisitionType,
            ...
            <div class="info-grid">
              <div>
                ${getDetailListHtml([
                  ...
                  { label: fullPaymentPlan ? 'Payment Timing' : '40% Collection Due', value: fullPaymentPlan ? 'Due by first collection milestone unless paid earlier' : fortyPercentDueDate.toLocaleDateString() },
                  { label: 'Reservation Fee', value: formatCurrency(reservationFeeAmount) },
                ])}
              </div>
              <div>
                ${getDetailListHtml([
  ...
  const handleExportOfficialReceipt = (payment: Contract['payments'][number]) => {
    ...
    if (!printWindow) {
      toast.error('Please allow pop-ups to generate the provisional receipt');
      return;
    }
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>${escapeHtml(payment.receiptNumber || 'Provisional Receipt')}</title>
          <style>
            ${PRINT_DOCUMENT_STYLES}
          </style>
        </head>
        <body>
            ...
            <div class="info-grid">
              ...
              <div>
                ${getDetailListHtml([
                  ...
                  { label: 'Provisional Receipt No.', value: payment.receiptNumber || '-' },
                ])}
              </div>
            </div>
          </section>
          ...
          <div class="document-note">
            This provisional receipt was generated by ${COMPANY_NAME} for accounting and client acknowledgment.
          </div>
        </div>
      </div>
        </body>
  ...
  const canManageStockroom = canUpdatePreparation && (isStockroom() || isAdmin());
  const isInventoryDepartmentViewer = isCreative() || isLinen() || isStockroom();
  ...
  const canEditDraftStockroomInventory = contract ? contract.status === 'draft' && (isStockroom() || isAdmin()) : false;
  ...
  const canConfirmStockroomSection = canConfirmFinalizationSections && (isStockroom() || isAdmin());
                          ...
                          {(item.alternativeSuggestions || []).length > 0 ? (
                            <div className="mt-3 rounded-lg border border-blue-200 bg-blue-50 px-3 py-3">
                              ...
                              <div className="mt-2 grid gap-2 sm:grid-cols-2">
                                {(item.alternativeSuggestions || []).map((alternative) => (
                                  <div key={alternative.itemId} className="rounded-lg border border-blue-100 bg-white p-2 text-sm">
                                    <div className="flex items-start gap-2">
                                      <button
                                        type="button"
                                        className={`h-12 w-12 shrink-0 overflow-hidden rounded-md bg-muted ${alternative.imageUrl ? 'cursor-zoom-in' : 'cursor-default'}`}
                                        onClick={() => alternative.imageUrl && setPreviewImage({ url: alternative.imageUrl, title: alternative.itemName })}
                                      >
                                        {alternative.imageUrl ? (
                                          <img src={alternative.imageUrl} alt={alternative.itemName} className="h-full w-full object-cover" />
                                        ) : (
                                          <div className="flex h-full items-center justify-center px-1 text-center text-[10px] text-muted-foreground">No image</div>
                                        )}
                                      </button>
                                      <div className="min-w-0 flex-1">
                                        <p className="truncate font-medium text-blue-950">{alternative.itemName}</p>
                                        <p className="text-xs text-blue-900/70">{alternative.itemCode || alternative.category || 'Alternative item'}</p>
                                        <div className="mt-1 flex flex-wrap gap-1">
                                          <Badge variant="outline">Available {alternative.availableQuantity}</Badge>
                                          {alternative.pricePerItem ? (
                                            <Badge variant="outline">{formatCurrency(alternative.pricePerItem)} each</Badge>
                                          ) : null}
                                          {alternative.canCoverFullRequest ? (
                                            <Badge className="bg-green-100 text-green-800">Can cover request</Badge>
                                          ) : null}
                                        </div>
                                      </div>
                                    </div>
  ...
  const canViewPreferencesTab = useBanquetFocusedContractView || (!useInventoryFocusedContractView && (isAdmin() || isSales() || isCreative() || isLinen() || isKitchen() || isBanquet()));
  ...
  const reservationFeeAmount = Number(contract.reservationFeeAmount) || RESERVATION_FEE_AMOUNT;
  ...
  const fortyPercentDueDate = getFortyPercentDueDate(contract.bookingDate);
  const fortyPercentFollowUpDate = getFortyPercentDueDate(contract.bookingDate);
  fortyPercentFollowUpDate.setMonth(fortyPercentFollowUpDate.getMonth() - 1);
  ...
  const fortyPercentDeadline = new Date(fortyPercentDueDate);
  fortyPercentDeadline.setHours(23, 59, 59, 999);
  const agingDeadline = new Date(agingEndsAt);
  ...
  const paymentHoldActive = contract.clientSigned && (finalBalancePastDue || fortyPercentPastDue) && !fullyPaid;
  ...
  const paymentPlanLabel = fullPaymentPlan ? 'Full Payment' : `${downPaymentPercent}% / ${finalPaymentPercent}% Collection`;
  const paymentRequirementLabel = fullPaymentPlan
    ? 'Full Payment Required'
    : finalBalancePastDue
      ? 'Full Balance Now Required'
      : `${downPaymentPercent}% Collection`;
  const paymentSummaryLabel = fullPaymentPlan ? 'Payment Required' : '40% Collection';
  ...
  const paymentPlanSummary = fullPaymentPlan
    ...
    : isPreSignatureStage
      ? `This records the agreed ${downPaymentPercent}% / ${finalPaymentPercent}% arrangement before signature. The PHP ${reservationFeeAmount.toLocaleString()} reservation fee and collection tracking start after signing.`
      : `PHP ${reservationFeeAmount.toLocaleString()} reservation fee validates the booking, ${downPaymentPercent}% is due by ${fortyPercentDueDate.toLocaleDateString()}, and the remaining ${finalPaymentPercent}% is due by ${finalBalanceDueDate.toLocaleDateString()}.`;
  ...
  const canViewStockroomInventorySection = isAdmin() || isLogistics() || isSales() || isStockroom();
  ...
  const canCreateCreativeProcurementRequest = !eventHasPassed && !isContractClosed && (isCreative() || isAdmin());
  const canCreateLinenProcurementRequest = !eventHasPassed && !isContractClosed && (isLinen() || isAdmin());
  const canCreateStockroomProcurementRequest = !eventHasPassed && !isContractClosed && (isLogistics() || isStockroom() || isAdmin());
  ...
  const finalizationItems: DepartmentReadiness[] = [
    ...
    {
      key: 'payments',
      label: isPreSignatureStage ? 'Payment Term' : 'Collection Milestone',
      ...
      detail: isPreSignatureStage
        ...
        : !contract.clientSigned
          ...
          : uncollectible
            ? `The 40% collection has passed the 30-day aging window ending ${agingEndsAt.toLocaleDateString()}.`
          : fortyPercentPastDue
            ? `The 40% collection was due on ${fortyPercentDueDate.toLocaleDateString()} and is now in the 30-day aging window.`
          : finalBalancePastDue
            ? `${formatCurrency(remainingBalance)} final balance is overdue since ${finalBalanceDueDate.toLocaleDateString()}.`
            : paymentRequirementMet
              ? fullPaymentPlan
                ? `${formatCurrency(totalPaid)} received. Full payment target met.`
                : `${formatCurrency(totalPaid)} received. 40% collection milestone met.`
              : fullPaymentPlan
                ? `${formatCurrency(paymentRequirementRemaining)} still needed to complete the full payment requirement.`
                : `${formatCurrency(paymentRequirementRemaining)} still needed to reach the ${downPaymentPercent}% collection milestone.`,
    },
    {
      key: 'signature',
      label: 'Client Signed',
    ...
    },
    {
      ...
      detail: ['approved', 'completed'].includes(contract.status)
        ...
        : uncollectible
          ? 'Accounting must resolve or mark the account uncollectible before operations can proceed.'
        : fortyPercentPastDue
          ? 'The 40% collection is overdue and must be resolved before preparation approval.'
        : finalBalancePastDue
          ? 'The final 60% balance must be posted before the event can proceed.'
  ...
  const finalizationStatusLabel = contract.status === 'draft'
    ...
    : contract.status === 'pending_client_signature'
      ...
      : contract.status === 'submitted'
        ? uncollectible ? 'Uncollectible Review' : fortyPercentPastDue ? 'Aging 30 Days' : paymentHoldActive ? 'Payment Hold' : paymentRequirementMet ? 'Awaiting Accounting Approval' : 'Awaiting Collection'
        : contract.status === 'accounting_review'
          ? uncollectible ? 'Uncollectible Review' : fortyPercentPastDue ? 'Aging 30 Days' : paymentHoldActive ? 'Payment Hold' : 'In Accounting Review'
        : 'Finalized';
  ...
  const finalizationSummary = contract.status === 'draft'
    ...
    : contract.status === 'submitted'
        ? paymentHoldActive
          ? uncollectible
            ? `The 40% collection passed the 30-day aging window on ${agingEndsAt.toLocaleDateString()}. Accounting should resolve the account before release.`
            : fortyPercentPastDue
              ? `The 40% collection was due on ${fortyPercentDueDate.toLocaleDateString()}. The account is aging until ${agingEndsAt.toLocaleDateString()}.`
              : `The final 60% balance was due on ${finalBalanceDueDate.toLocaleDateString()}. The event is on payment hold until full payment is posted.`
          : paymentRequirementMet
          ...
          : fullPaymentPlan
            ...
            : `The client has signed, but the required ${downPaymentPercent}% collection (${formatCurrency(requiredDownPayment)}) must be posted before preparation can be approved.`
        : contract.status === 'accounting_review'
          ? paymentHoldActive
            ? uncollectible
              ? `The 40% collection passed the 30-day aging window on ${agingEndsAt.toLocaleDateString()}. Accounting should resolve the account before release.`
            ...
            <CardContent className="space-y-4">
              {showPreparationReadiness ? (
                ...
                <>
                  ...
                  <div className={`rounded-lg border px-4 py-3 text-sm ${fullPaymentPlan ? (fullyPaid ? 'border-green-200 bg-green-50 text-green-900' : 'border-slate-200 bg-slate-50 text-slate-700') : uncollectible || finalBalancePastDue ? 'border-red-200 bg-red-50 text-red-900' : fortyPercentPastDue ? 'border-amber-200 bg-amber-50 text-amber-900' : 'border-slate-200 bg-slate-50 text-slate-700'}`}>
                    {fullPaymentPlan ? (
                      <>
                      ...
                      <>
                        Reservation fee: <strong>{formatCurrency(reservationFeeAmount)}</strong>. 40% collection: <strong>{formatCurrency(requiredDownPayment)}</strong>{' '}
                        due on <strong>{fortyPercentDueDate.toLocaleDateString()}</strong>. Final {finalPaymentPercent}% due on <strong>{finalBalanceDueDate.toLocaleDateString()}</strong>.
                        {fullyPaid
                          ? ' This contract is already fully paid.'
                          : uncollectible
                            ? ' The 40% collection is beyond the 30-day aging window.'
                            : fortyPercentPastDue
                              ? ' The 40% collection is now in the 30-day aging window.'
                              : finalBalancePastDue
                                ? ' The final balance due date has already passed and the event is on payment hold.'
                                : ''}
                      </>
                    )}
                  </div>
              ...
              <Card className={useBanquetFocusedContractView ? 'xl:col-span-1' : 'md:col-span-2'}>
                ...
                <CardContent className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{contract.venue?.capacity ? `${contract.venue.capacity.toLocaleString()} pax` : '-'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Contact</span>
              ...
              <>
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    {operationsSummary?.materialFreeze ? (
                      <div className={`rounded-lg border px-4 py-3 text-sm ${
                        ...
                        <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                          <p className="font-medium">{operationsSummary.materialFreeze.label}</p>
                          <Badge variant="outline">
                            {operationsSummary.materialFreeze.daysUntilEvent === 0
                              ? 'Event today'
                              : `${operationsSummary.materialFreeze.daysUntilEvent ?? 0} day(s) to event`}
                          </Badge>
                        </div>
                        <p className="mt-1">{operationsSummary.materialFreeze.note}</p>
                      </div>
                    ) : null}
                    <p className="text-sm text-muted-foreground">
                      {eventHasPassed
                            ...
                            <div className="mt-2 space-y-3">
                              ...
                              {(item.alternativeSuggestions || []).length > 0 ? (
                                <div className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2">
                                  <p className="text-xs font-semibold uppercase tracking-[0.14em] text-blue-900">Suggested in-house alternatives</p>
                                  <div className="mt-2 flex flex-wrap gap-2">
                                    {(item.alternativeSuggestions || []).map((alternative) => (
                                      <Badge key={alternative.itemId} variant="outline" className="bg-white">
                                        {alternative.itemName}: {alternative.availableQuantity} available
                                      </Badge>
                                    ))}
                                  </div>
                                </div>
              ...
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                {canPostPayments && (
                  <Dialog open={paymentDialogOpen} onOpenChange={setPaymentDialogOpen}>
                    ...
                    <DialogContent>
                      <DialogHeader>
                        <DialogTitle>Record Payment And Provisional Receipt</DialogTitle>
                      </DialogHeader>
                      <div className="space-y-4 pt-4">
                        ...
                        <div className="space-y-2">
                          <Label>Provisional Receipt Number</Label>
                          <Input
                            value={receiptNumber}
                            onChange={(e) => setReceiptNumber(e.target.value)}
                            placeholder="Enter provisional receipt number"
                          />
                        </div>
                        <div className="space-y-2">
              ...
              <CardContent className="space-y-4">
                      ...
                      <div className="mt-4 rounded-lg border border-dashed p-4">
                        ...
                        >
                          <label className="flex items-start gap-3 rounded-lg border p-3 cursor-pointer">
                            ...
                            <div className="space-y-1">
                              <Label htmlFor="payment-plan-split" className="cursor-pointer font-medium">40 / 60 Payment Term</Label>
                              <p className="text-sm text-muted-foreground">
                                PHP 30,000 reservation fee validates the booking, 40% is due two months after booking, then the remaining 60% is due two months before the event.
                              </p>
                            </div>
                          </label>
                          <label className="flex items-start gap-3 rounded-lg border p-3 cursor-pointer">
                            ...
                            <div className="space-y-1">
                              ...
                              <p className="text-sm text-muted-foreground">
                                Client may settle the full contract amount early at the first collection milestone.
                              </p>
                            </div>
                          </label>
                        </RadioGroup>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <div className={`rounded-lg border px-4 py-3 text-sm ${isPreSignatureStage ? 'border-slate-200 bg-slate-50 text-slate-700' : paymentRequirementMet ? 'border-green-200 bg-green-50 text-green-900' : 'border-amber-200 bg-amber-50 text-amber-900'}`}>
                    {isPreSignatureStage
                      ? 'This section records the agreed payment arrangement before signing. Receipt posting starts after the signed contract is pushed to accounting.'
                      : fullyPaid
                      ? 'Contract fully paid. The payment requirement for preparation is complete.'
                      : uncollectible
                        ? `The 40% collection passed the aging window on ${agingEndsAt.toLocaleDateString()}. Accounting should resolve the account before release.`
                      : fortyPercentPastDue
                      ...
                      : paymentRequirementMet
                        ? fullPaymentPlan
                          ...
                          : `40% collection milestone met. ${formatCurrency(remainingBalance)} remains for the final ${finalPaymentPercent}% balance.`
                        : fullPaymentPlan
                          ? `${formatCurrency(paymentRequirementRemaining)} is still needed to complete the full payment required before preparation.`
                          : `${formatCurrency(paymentRequirementRemaining)} is still needed to reach the required ${downPaymentPercent}% collection.`}
                  </div>
                  <div className={`rounded-lg border px-4 py-3 text-sm ${fullPaymentPlan ? (fullyPaid ? 'border-green-200 bg-green-50 text-green-900' : 'border-slate-200 bg-slate-50 text-slate-700') : finalBalancePastDue ? 'border-red-200 bg-red-50 text-red-900' : fortyPercentPastDue ? 'border-amber-200 bg-amber-50 text-amber-900' : 'border-slate-200 bg-slate-50 text-slate-700'}`}>
                    {fullPaymentPlan ? (
                      <>
                      ...
                      <>
                        ...
                        {isPreSignatureStage
                          ...
                          : fullyPaid
                            ? ' Fully paid.'
                            : finalBalancePastDue
                              ? ' The final due date has already passed and the event is on payment hold until the balance is settled.'
                              : fortyPercentPastDue
                                ? ' The 40% collection is currently in aging follow-up.'
                              : ''}
                      </>
                    )}
                  </div>
                      ...
                      >
                        <div>
                          ...
                          {payment.receiptNumber ? (
                            <p className="text-sm text-muted-foreground">PR No.: {payment.receiptNumber}</p>
                          ) : null}
                          {payment.reference ? (
            ...
            <div className="space-y-4 pt-2">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={procurementRequestForm.requisitionType}
                    onValueChange={(value) => setProcurementRequestForm((current) => ({ ...current, requisitionType: value }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="rounded-lg border bg-muted/20 p-3 text-sm text-muted-foreground">
                {procurementRequestForm.requisitionType === 'emergency_requisition'
                  ? 'Emergency requests are allowed inside the standard 7-day lead time and should be used for same-week shortages.'
