...
export default function Dashboard() {
  ...
  const canViewContracts = ['sales', 'accounting', 'creative', 'linen', 'stockroom', 'kitchen', 'banquet_supervisor', 'logistics', 'admin'].includes(role || '');
  ...
  const fetchDashboardData = async () => {
    try {
      const [statsData, contractsData] = await Promise.all([
        ...
        canViewContracts ? api.getContracts() : Promise.resolve([])
      ]);
      setStats(statsData);
  ...
  const getDepartmentDashboard = () => {
    switch (role) {
      ...
      case 'stockroom':
        return { label: 'Stockroom Dashboard', href: '/stockroom', color: 'bg-amber-600' };
      case 'creative':
