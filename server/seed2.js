...
const { getDefaultMonthlyBudgetTemplate, getPeriodMonth } = require('./utils/financeBudgeting');
...
const buildDemoSla = (requisitionType, neededBy) => {
  const daysUntilNeeded = getDaysUntilDate(neededBy);
  const isEmergency = requisitionType === 'emergency_requisition';

  return {
    requiredLeadDays: isEmergency ? 0 : 7,
    daysUntilNeeded,
    status: isEmergency ? 'rush' : 'on_track',
    reviewNote: isEmergency
      ? 'Emergency requisition allowed inside the standard 7-day lead time. MCM-only approval path should be used.'
      : 'Standard requisitions require at least 7 days before the needed-by date.',
    computedAt: new Date(),
  };
};
...
async function seedDemoData() {
  ...
  const inventoryConflictEventDate = addDays(preferredEventDate, 14);
  ...
  const accountingUser = ensuredUsers.accounting;
  const purchasingUser = ensuredUsers.purchasing;
  const stockroomUser = ensuredUsers.stockroom;
  const linenUser = ensuredUsers.linen;
  ...
  const stockroomRectTable = (await upsertDocument(StockroomInventory, { itemCode: 'TAB-DEMO-2603-0002' }, {
    itemCode: 'TAB-DEMO-2603-0002',
    name: 'Rectangular Banquet Table 6ft',
    category: 'Table',
    subcategory: 'Dining Table',
    description: 'Rectangular banquet table that can be used when round tables are already reserved.',
    quantity: 80,
    ...
    condition: 'good',
    status: 'available',
    dimensions: { length: 183, width: 76, height: 76, weight: 22 },
    storageLocation: { warehouse: 'Main Warehouse', section: 'Tables', shelf: 'T-02' },
    rentalPricePerDay: 420,
    replacementCost: 6200,
    supplier: { name: 'Metro Event Furnishings', contact: '09181112223', email: 'orders@metrofurnishings.ph' },
    purchaseDate: addMonths(today, -8),
    notes: `${DEMO_TAG} Alternative table stock for date-specific shortage demo.`,
  })).doc;
  ...
  console.log('\nEnsuring demo suppliers and finance budgets...');
  const metroSupplier = (await upsertDocument(Supplier, { name: 'Metro Event Furnishings' }, {
    name: 'Metro Event Furnishings',
    contactPerson: 'Carlo Lim',
    phone: '09181112223',
    email: 'orders@metrofurnishings.ph',
    address: 'Warehouse Row, Lipa City',
    city: 'Lipa City',
    province: 'Batangas',
    serviceAreas: ['Lipa City', 'Batangas', 'Tanauan'],
    departments: ['stockroom'],
    requestTypes: ['purchase', 'rental'],
    supportedCategories: ['Table', 'Chair', 'Equipment'],
    supportedKeywords: ['table', 'chair', 'banquet', 'truck', 'tent'],
    isPreferred: true,
    priority: 20,
    notes: `${DEMO_TAG} Preferred demo supplier for warehouse assets and emergency rentals.`,
    isActive: true,
  })).doc;

  const linenSupplier = (await upsertDocument(Supplier, { name: 'Linen Source Manila' }, {
    name: 'Linen Source Manila',
    contactPerson: 'Mia Santos',
    phone: '09190001122',
    email: 'sales@linensourcemanila.demo',
    address: 'Textile District, Manila',
    city: 'Manila',
    province: 'Metro Manila',
    serviceAreas: ['Metro Manila', 'Batangas', 'Lipa City'],
    departments: ['linen'],
    requestTypes: ['purchase', 'rental'],
    supportedCategories: ['Tablecloth', 'Napkin', 'Sash'],
    supportedKeywords: ['linen', 'tablecloth', 'napkin', 'sash'],
    isPreferred: true,
    priority: 18,
    notes: `${DEMO_TAG} Demo supplier for linen replenishment requests.`,
    isActive: true,
  })).doc;

  const budgetMonths = [...new Set([
    getPeriodMonth(today),
    getPeriodMonth(addDays(today, 9)),
    getPeriodMonth(preferredEventDate),
  ])];

  for (const periodMonth of budgetMonths) {
    const template = getDefaultMonthlyBudgetTemplate();
    await upsertDocument(FinanceBudget, { periodMonth }, {
      ...template,
      periodMonth,
      totalBudget: 500000,
      sourceOfFunds: 'management_approved',
      status: 'active',
      notes: `${DEMO_TAG} Active demo budget for Phase 3 procurement approval testing.`,
      preparedBy: accountingUser?._id || adminUser?._id || null,
      approvedBy: adminUser?._id || accountingUser?._id || null,
      approvedAt: new Date(),
    });
  }

  console.log(`  ensured ${budgetMonths.length} active finance budget month(s) and 2 demo suppliers`);
  ...
  const confirmedSections = {
    ...
    payments: createConfirmedSection(adminUser?._id || null, signingDate),
  ...
  const submittedContract = await upsertDocument(Contract, { contractNumber: 'DEMO-CONTRACT-RELEASE-THIS-WEEK' }, {
    ...
    eventDate: atTime(preferredEventDate, 12, 0),
    ...
    reservationFeeAmount: 30000,
    payments: [
      { amount: 30000, date: addDays(today, -1), method: 'bank_transfer', reference: 'DEMO-RES-FEE-30000', receiptNumber: 'OR-DEMO-REL-0001', receiptIssuedBy: 'Juan Carlos', receiptGeneratedAt: addDays(today, -1), status: 'completed' },
    ],
  ...
  const inventoryLockContract = await upsertDocument(Contract, { contractNumber: 'DEMO-CONTRACT-INVENTORY-LOCK' }, {
    contractNumber: 'DEMO-CONTRACT-INVENTORY-LOCK',
    status: 'approved',
    currentDepartment: 'all',
    menuTasting: null,
    clientSigned: true,
    clientSignedAt: addDays(today, -2),
    clientName: 'Alvarez Foundation Gala',
    clientContact: '09171230006',
    clientEmail: 'events@alvarezgala.demo',
    clientAddress: {
      street: '22 Bonifacio Drive',
      city: 'Lipa City',
      province: 'Batangas',
      zipCode: '4217',
    },
    clientType: 'corporate',
    eventDate: atTime(inventoryConflictEventDate, 18, 0),
    bookingDate: addMonths(today, -1),
    venue: {
      name: 'The Orchard Pavilion',
      address: 'Lipa City, Batangas',
      contact: '09176660000',
      notes: `${DEMO_TAG} Approved event that intentionally reserves most round tables for the conflict demo.`,
    },
    eventType: 'charity gala',
    packageSelected: 'premium',
    menuDetails: [
      { category: 'Main Course', item: 'Roast Beef', quantity: 450, confirmed: true },
      { category: 'Main Course', item: 'Buttered Chicken', quantity: 450, confirmed: true },
      { category: 'Dessert', item: 'Mango Float', quantity: 450, confirmed: true },
    ],
    totalPacks: 450,
    preferredColor: 'Black and Gold',
    napkinType: 'Satin',
    tableSetup: 'round',
    backdropRequirements: 'Formal stage backdrop',
    specialRequests: 'Donor table layout',
    packagePrice: 280000,
    ...
    reservationFeeAmount: 30000,
    payments: [
      { amount: 360000, date: addDays(today, -1), method: 'bank_transfer', reference: 'DEMO-LOCK-FULL-0001', receiptNumber: 'OR-DEMO-LOCK-0001', receiptIssuedBy: 'Juan Carlos', receiptGeneratedAt: addDays(today, -1), status: 'completed' },
    ],
    paymentStatus: 'paid',
    logisticsAssignment: { driver: null, truck: null, assignmentStatus: 'pending', notes: `${DEMO_TAG} Inventory lock event for same-date reservation demo.` },
    equipmentChecklist: [
      { itemId: String(stockroomTable._id), item: stockroomTable.name, itemCode: stockroomTable.itemCode, category: stockroomTable.category, quantity: 55, unitPrice: stockroomTable.rentalPricePerDay, notes: 'Large gala round table layout', status: 'pending' },
    ],
    cookingLocation: 'commissary',
    ingredientStatus: 'pending',
    linenRequirements: [],
    ...
    departmentProgress: { sales: 100, accounting: 100, logistics: 10, banquet: 10, kitchen: 10, purchasing: 10, creative: 10, linen: 10 },
    sectionConfirmations: confirmedSections,
    internalNotes: `${DEMO_TAG} This contract reserves 55 of 60 round tables on the conflict demo date.`,
  });

  console.log(`  ${inventoryLockContract.created ? 'created' : 'updated'} DEMO-CONTRACT-INVENTORY-LOCK`);

  const inventoryConflictContract = await upsertDocument(Contract, { contractNumber: 'DEMO-CONTRACT-INVENTORY-CONFLICT' }, {
    contractNumber: 'DEMO-CONTRACT-INVENTORY-CONFLICT',
    status: 'submitted',
    currentDepartment: 'accounting',
    menuTasting: null,
    clientSigned: true,
    clientSignedAt: addDays(today, -1),
    clientName: 'Mercado Product Launch',
    clientContact: '09171230007',
    clientEmail: 'launch@mercado.demo',
    clientAddress: {
      street: '8 JP Laurel Avenue',
      city: 'Lipa City',
      province: 'Batangas',
      zipCode: '4217',
    },
    clientType: 'corporate',
    eventDate: atTime(inventoryConflictEventDate, 11, 0),
    bookingDate: addMonths(today, -1),
    venue: {
      name: 'The Orchard Pavilion',
      address: 'Lipa City, Batangas',
      contact: '09176660000',
      notes: `${DEMO_TAG} Same-day conflict demo requesting round tables already reserved by another event.`,
    },
    eventType: 'product launch',
    packageSelected: 'standard',
    menuDetails: [
      { category: 'Main Course', item: 'Fish Fillet', quantity: 160, confirmed: true },
      { category: 'Main Course', item: 'Buttered Chicken', quantity: 160, confirmed: true },
      { category: 'Dessert', item: 'Fruit Salad', quantity: 160, confirmed: true },
    ],
    totalPacks: 160,
    preferredColor: 'White and Green',
    napkinType: 'Cotton',
    tableSetup: 'round',
    backdropRequirements: 'Product stage backdrop',
    specialRequests: 'Demo station near entrance',
    packagePrice: 72000,
    ...
    reservationFeeAmount: 30000,
    payments: [
      { amount: 36000, date: addDays(today, -1), method: 'bank_transfer', reference: 'DEMO-CONFLICT-40-0001', receiptNumber: 'OR-DEMO-CONFLICT-0001', receiptIssuedBy: 'Juan Carlos', receiptGeneratedAt: addDays(today, -1), status: 'completed' },
    ],
    paymentStatus: 'partially_paid',
    logisticsAssignment: { driver: null, truck: null, assignmentStatus: 'pending', notes: `${DEMO_TAG} Approval should be blocked until shortage is swapped or covered.` },
    equipmentChecklist: [
      { itemId: String(stockroomTable._id), item: stockroomTable.name, itemCode: stockroomTable.itemCode, category: stockroomTable.category, quantity: 20, unitPrice: stockroomTable.rentalPricePerDay, notes: 'Requested round tables; should show rectangular table alternatives', status: 'pending' },
    ],
    cookingLocation: 'commissary',
    ingredientStatus: 'pending',
    linenRequirements: [],
    ...
    estimatedVehicles: 2,
    submittedAt: addDays(today, -1),
    departmentProgress: { sales: 100, accounting: 50, logistics: 0, banquet: 0, kitchen: 0, purchasing: 0, creative: 0, linen: 0 },
    sectionConfirmations: confirmedSections,
    internalNotes: `${DEMO_TAG} Open this contract to demo date-specific stock locking, shortage alert, and available rectangular table alternatives.`,
  });

  console.log(`  ${inventoryConflictContract.created ? 'created' : 'updated'} DEMO-CONTRACT-INVENTORY-CONFLICT`);

  const agingContract = await upsertDocument(Contract, { contractNumber: 'DEMO-CONTRACT-AGING-40' }, {
    contractNumber: 'DEMO-CONTRACT-AGING-40',
    status: 'submitted',
    currentDepartment: 'accounting',
    menuTasting: null,
    clientSigned: true,
    clientSignedAt: addMonths(today, -1),
    clientName: 'Batangas Medical Society',
    clientContact: '09171230005',
    clientEmail: 'secretariat@batangasmed.demo',
    clientAddress: {
      street: '12 Laurel Highway',
      city: 'Batangas City',
      province: 'Batangas',
      zipCode: '4200',
    },
    clientType: 'corporate',
    eventDate: atTime(addDays(preferredEventDate, 45), 10, 0),
    bookingDate: addDays(addMonths(today, -2), -10),
    venue: {
      name: 'Batangas Grand Ballroom',
      address: 'Batangas City, Batangas',
      contact: '09175551111',
      notes: `${DEMO_TAG} Accounting demo account currently inside the 30-day aging window.`,
    },
    eventType: 'medical conference',
    packageSelected: 'standard',
    menuDetails: [
      { category: 'Main Course', item: 'Chicken Cordon Bleu', quantity: 80, confirmed: true },
      { category: 'Main Course', item: 'Fish Fillet', quantity: 80, confirmed: true },
      { category: 'Side', item: 'Steamed Rice', quantity: 80, confirmed: true },
      { category: 'Dessert', item: 'Buko Pandan', quantity: 80, confirmed: true },
    ],
    totalPacks: 80,
    preferredColor: 'White and Sage',
    napkinType: 'Cotton',
    tableSetup: 'round',
    backdropRequirements: 'Simple conference stage backdrop',
    specialRequests: 'AM snacks and lunch service',
    creativeRequirements: {
      theme: 'Professional conference',
      colorPalette: ['White', 'Sage'],
      backdropType: 'Custom',
      tableCenterpieces: 'Low greenery centerpieces',
    },
    creativeAssets: [
      { itemId: String(creativeCenterpiece._id), item: creativeCenterpiece.name, itemCode: creativeCenterpiece.itemCode, category: 'Table Decor', quantity: 8, status: 'pending', notes: 'Conference table centerpieces', cost: 9600, pricePerItem: 1200 },
    ],
    packagePrice: 76000,
    ...
    reservationFeeAmount: 30000,
    payments: [
      { amount: 30000, date: addMonths(today, -2), method: 'check', reference: 'DEMO-AGING-RES-0001', receiptNumber: 'OR-DEMO-AGING-0001', receiptIssuedBy: 'Juan Carlos', receiptGeneratedAt: addMonths(today, -2), status: 'completed' },
    ],
    paymentStatus: 'partially_paid',
    logisticsAssignment: {
      driver: null,
      truck: null,
      assignmentStatus: 'pending',
      notes: `${DEMO_TAG} Accounting aging report demo; do not release until the 40% balance is posted.`,
    },
    equipmentChecklist: [
      { itemId: String(stockroomChair._id), item: stockroomChair.name, itemCode: stockroomChair.itemCode, category: stockroomChair.category, quantity: 80, unitPrice: stockroomChair.rentalPricePerDay, notes: 'Conference seating', status: 'pending' },
      { itemId: String(stockroomTable._id), item: stockroomTable.name, itemCode: stockroomTable.itemCode, category: stockroomTable.category, quantity: 10, unitPrice: stockroomTable.rentalPricePerDay, notes: 'Round dining tables', status: 'pending' },
    ],
    cookingLocation: 'commissary',
    ingredientStatus: 'pending',
    linenRequirements: [
      { itemId: String(linenTablecloth._id), type: linenTablecloth.name, itemCode: linenTablecloth.itemCode, category: linenTablecloth.category, size: linenTablecloth.size, material: linenTablecloth.material, color: linenTablecloth.color, quantity: 10, unitPrice: 120, notes: 'Guest tables', status: 'pending' },
      { itemId: String(linenNapkin._id), type: linenNapkin.name, itemCode: linenNapkin.itemCode, category: linenNapkin.category, size: linenNapkin.size, material: linenNapkin.material, color: linenNapkin.color, quantity: 80, unitPrice: 35, notes: 'Guest place settings', status: 'pending' },
    ],
    linenStatus: 'pending',
    estimatedWaiters: 8,
    estimatedVehicles: 1,
    submittedAt: addMonths(today, -1),
    departmentProgress: { sales: 100, accounting: 10, logistics: 0, banquet: 0, kitchen: 0, purchasing: 0, creative: 0, linen: 0 },
    sectionConfirmations: confirmedSections,
    internalNotes: `${DEMO_TAG} Signed contract with reservation fee only; seeded to demonstrate AR aging for the 40% collection milestone.`,
  });

  console.log(`  ${agingContract.created ? 'created' : 'updated'} DEMO-CONTRACT-AGING-40`);
  ...
  const closeReadyContract = await upsertDocument(Contract, { contractNumber: 'DEMO-CONTRACT-CLOSE-READY' }, {
    ...
    reservationFeeAmount: 30000,
    payments: [
      { amount: 67200, date: closeReadyApprovalDate, method: 'bank_transfer', reference: 'DEMO-CLOSE-40-0001', receiptNumber: 'OR-DEMO-CLOSE-0001', receiptIssuedBy: 'Juan Carlos', receiptGeneratedAt: closeReadyApprovalDate, status: 'completed' },
      { amount: 100800, date: closeReadyFinalPaymentDate, method: 'cash', reference: 'DEMO-CLOSE-FINAL-0002', receiptNumber: 'OR-DEMO-CLOSE-0002', receiptIssuedBy: 'Juan Carlos', receiptGeneratedAt: closeReadyFinalPaymentDate, status: 'completed' },
    ],
    paymentStatus: 'paid',
    logisticsAssignment: {
  ...
  console.log('\nEnsuring demo procurement requests...');
  const standardNeededBy = addDays(today, 9);
  const emergencyNeededBy = atTime(eventDay, 8, 0);
  const releaseReadyNeededBy = addDays(today, 10);

  const standardRequest = await upsertDocument(ProcurementRequest, { requestNumber: 'DEMO-PR-STANDARD-7DAY' }, {
    requestNumber: 'DEMO-PR-STANDARD-7DAY',
    status: 'requested',
    ...
    contract: null,
    eventDate: null,
    neededBy: standardNeededBy,
    inventoryModel: 'StockroomInventory',
    inventoryItem: stockroomAudio._id,
    itemName: stockroomAudio.name,
    itemCode: stockroomAudio.itemCode,
    itemCategory: stockroomAudio.category,
    requestedQuantity: 3,
    shortageQuantity: 3,
    requestReason: `${DEMO_TAG} Standard 7-day item requisition for additional PA systems before next scheduled setup.`,
    requestNotes: 'Demo this in Purchasing: create the budget request, then watch it move to Accounting.',
    sla: buildDemoSla('item_requisition', standardNeededBy),
    quote: {},
    accounting: { status: 'pending' },
    releaseAuthorization: {
      status: 'locked',
      notes: 'Awaiting purchasing quote and accounting approval before release.',
    },
    fulfillment: {},
    createdBy: stockroomUser?._id || adminUser?._id || null,
    updatedBy: stockroomUser?._id || adminUser?._id || null,
  });

  const emergencyRequest = await upsertDocument(ProcurementRequest, { requestNumber: 'DEMO-PR-EMERGENCY-ACCOUNTING' }, {
    requestNumber: 'DEMO-PR-EMERGENCY-ACCOUNTING',
    ...
    contract: approvedContract.doc._id,
    eventDate: approvedContract.doc.eventDate,
    neededBy: emergencyNeededBy,
    inventoryModel: 'StockroomInventory',
    inventoryItem: stockroomTent._id,
    itemName: stockroomTent.name,
    itemCode: stockroomTent.itemCode,
    itemCategory: stockroomTent.category,
    requestedQuantity: 1,
    shortageQuantity: 1,
    requestReason: `${DEMO_TAG} Emergency rental request for same-week event weather coverage.`,
    requestNotes: `Linked to ${approvedContract.doc.contractNumber}; use this to demo the accounting release checkpoint.`,
    sla: buildDemoSla('emergency_requisition', emergencyNeededBy),
    quote: {
      supplier: metroSupplier._id,
      supplierName: metroSupplier.name,
      supplierContact: `${metroSupplier.contactPerson} | ${metroSupplier.phone}`,
      supplierEmail: metroSupplier.email,
      quotedUnitPrice: 11500,
      quotedTotal: 11500,
      expectedFulfillmentDate: emergencyNeededBy,
      rentalStartDate: emergencyNeededBy,
      rentalEndDate: addDays(emergencyNeededBy, 1),
      notes: 'Emergency rental quotation; accounting must approve before funds can be released.',
      submittedAt: addDays(today, -1),
      submittedBy: purchasingUser?._id || adminUser?._id || null,
    },
    accounting: { status: 'pending' },
    releaseAuthorization: {
      status: 'locked',
      notes: 'No signature, no release: waiting for accounting budget approval.',
    },
    fulfillment: {},
    createdBy: stockroomUser?._id || adminUser?._id || null,
    updatedBy: purchasingUser?._id || adminUser?._id || null,
  });

  const releaseReadyRequest = await upsertDocument(ProcurementRequest, { requestNumber: 'DEMO-PR-RELEASE-AUTHORIZED' }, {
    requestNumber: 'DEMO-PR-RELEASE-AUTHORIZED',
    status: 'approved',
    department: 'linen',
    requestType: 'purchase',
    ...
    source: 'inventory_low_stock',
    sourceSection: 'linen',
    contract: submittedContract.doc._id,
    eventDate: submittedContract.doc.eventDate,
    neededBy: releaseReadyNeededBy,
    ...
    shortageQuantity: 80,
    requestReason: `${DEMO_TAG} Linen replenishment already approved by accounting; ready for proof-of-purchase demo.`,
    requestNotes: 'Demo this in Purchasing: submit proof, then Accounting confirms the expense.',
    sla: buildDemoSla('purchase_requisition', releaseReadyNeededBy),
    quote: {
      supplier: linenSupplier._id,
      supplierName: linenSupplier.name,
      supplierContact: `${linenSupplier.contactPerson} | ${linenSupplier.phone}`,
      supplierEmail: linenSupplier.email,
      quotedUnitPrice: 78,
      quotedTotal: 6240,
      expectedFulfillmentDate: addDays(today, 8),
      notes: 'Standard linen purchase request with accounting release already authorized.',
      submittedAt: addDays(today, -2),
      submittedBy: purchasingUser?._id || adminUser?._id || null,
    },
    accounting: {
      ...
      reviewedBy: accountingUser?._id || adminUser?._id || null,
      notes: `${DEMO_TAG} Budget approved for linen replenishment.`,
      reviewChecklist: {
        inventoryNeedValidated: true,
    ...
    releaseAuthorization: {
      status: 'authorized',
      authorizedAt: addDays(today, -1),
      authorizedBy: accountingUser?._id || adminUser?._id || null,
      signatureLabel: 'Digitally approved by Accounting Manager',
      notes: 'No signature, no release checkpoint cleared.',
    },
    fulfillment: {},
    createdBy: linenUser?._id || adminUser?._id || null,
    updatedBy: accountingUser?._id || adminUser?._id || null,
  });

  console.log(`  ${standardRequest.created ? 'created' : 'updated'} DEMO-PR-STANDARD-7DAY`);
  console.log(`  ${emergencyRequest.created ? 'created' : 'updated'} DEMO-PR-EMERGENCY-ACCOUNTING`);
  console.log(`  ${releaseReadyRequest.created ? 'created' : 'updated'} DEMO-PR-RELEASE-AUTHORIZED`);
  ...
  console.log('  Inventory lock contract: DEMO-CONTRACT-INVENTORY-LOCK');
  console.log('  Inventory conflict contract: DEMO-CONTRACT-INVENTORY-CONFLICT');
  console.log('  Accounting aging contract: DEMO-CONTRACT-AGING-40');
  console.log(`  Close-ready contract: DEMO-CONTRACT-CLOSE-READY (${formatDate(atTime(closeReadyEventDay, 17, 0))})`);
  console.log('  Procurement standard request: DEMO-PR-STANDARD-7DAY');
  console.log('  Procurement accounting approval request: DEMO-PR-EMERGENCY-ACCOUNTING');
  console.log('  Procurement release-ready request: DEMO-PR-RELEASE-AUTHORIZED');
