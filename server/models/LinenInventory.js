...
linenInventorySchema.pre('save', async function() {
  if (this.isNew || !this.isModified('quantity') || this.isModified('availableQuantity')) {
    return;
  }

  const previous = await this.constructor.findById(this._id).select('quantity availableQuantity').lean();
  if (!previous) {
    this.availableQuantity = this.quantity;
    return;
  }
  ...
  const previousAvailableQuantity = Number(previous.availableQuantity);
  const safePreviousAvailable = Number.isFinite(previousAvailableQuantity) ? previousAvailableQuantity : previousQuantity;
  const quantityDelta = (Number(this.quantity) || 0) - previousQuantity;

  this.availableQuantity = Math.max(0, Math.min(Number(this.quantity) || 0, safePreviousAvailable + quantityDelta));
});
